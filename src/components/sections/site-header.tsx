...
const NAV = [
  ...
  { label: "Operating", href: "#operating-model" },
  { label: "Journey", href: "#delivery-journey" },
  { label: "Capabilities", href: "#capabilities" },
  { label: "GTM", href: "#gtm-enablement" },
  { label: "CVP", href: "#product-tiers" },
  { label: "Content", href: "#content-bundles" },
  { label: "Combo + Content", href: "#combo-with-content" },
  { label: "Combo", href: "#combo-offers" },
  { label: "Voice / Data / SMS", href: "#single-service" },
...
export default function SiteHeader() {
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur">
      ...
      {/* Row 2: Section nav — wraps to 2 lines on narrower screens */}
      <nav className="mx-auto hidden max-w-6xl flex-wrap items-center justify-center gap-x-1 gap-y-0.5 border-t border-slate-100 px-4 py-2 md:flex">
        {NAV.map((n) => (
          <a
            key={n.href}
            href={n.href}
            className="whitespace-nowrap rounded-full px-2.5 py-1 text-[11px] font-medium text-slate-600 transition hover:bg-slate-100 hover:text-[#B91C1C]"
          >
            {n.label}
          </a>
        ))}
