...
export default function GtmEnablement() {
  return (
    <section id="gtm-enablement" className="bg-[#F8FAFC] py-24 md:py-32">
      <div className="mx-auto max-w-6xl px-6 md:px-10">
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {gtmEnablement.phases.map((p, i) => (
            <div
              key={p.phase}
              className="relative flex flex-col rounded-2xl border border-slate-200 bg-white p-7 transition hover:-translate-y-0.5 hover:border-[#B91C1C]/30 hover:shadow-[0_14px_40px_-20px_rgba(185,28,28,0.25)]"
            >
              <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-lg bg-[#B91C1C]/10 text-sm font-bold text-[#B91C1C]">
                {String(i + 1).padStart(2, "0")}
              </div>
              <div className="text-xs font-medium uppercase tracking-[0.2em] text-[#B91C1C]">
                {p.phase}
              </div>
              ...
              <ul className="mt-4 space-y-2">
                {p.activities.map((a) => (
                  ...
                  >
                    <span className="mt-1 h-1 w-1 shrink-0 rounded-full bg-[#B91C1C]" />
                    {a}
                  </li>
                ))}
              </ul>
        ...
        <div className="mt-20">
          ...
          <p className="mt-3 max-w-3xl text-base leading-relaxed text-slate-600">
            Beyond the MVNO platform and marketing program, Arena has access to DSG&apos;s full
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2">
            {gtmEnablement.supportingServices.map((s) => (
              <div
                key={s.title}
                className="flex items-start gap-4 rounded-2xl border border-slate-200 bg-white p-6 transition hover:border-[#B91C1C]/20 hover:shadow-[0_14px_40px_-20px_rgba(185,28,28,0.15)]"
              >
